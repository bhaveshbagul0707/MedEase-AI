...
export default function PdfChatPage() {
  ...
  async function loadFiles() {
    try {
      const response = await api.get<APIResponse<{ items: UploadedFile[] }>>('/files/')
      setFiles(response.data.data.items)
    } catch {
      toast.error('Unable to load your PDF library')
    }
  }
  ...
  async function ask() { if (!fileId || !question.trim()) return; setAsking(true); try { const response = await api.post<{ answer: string }>(`/knowledge/files/${fileId}/chat`, { question }); setAnswer(response.data.answer) } catch { toast.error('Unable to query this PDF') } finally { setAsking(false) } }
